    ...
    <div v-else>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <BaseCard class="bg-gradient-to-br from-blue-500 to-blue-600 text-white border-0">
          <div class="flex items-center justify-between">
            <div>
              <h3 class="text-sm font-medium opacity-90 mb-2">Total em Metas</h3>
              <p class="text-3xl font-bold">{{ formatCurrency(stats.totalGoals) }}</p>
              <p class="text-xs opacity-75 mt-1">{{ stats.goalsCount }} metas ativas</p>
            </div>
        ...
        <BaseCard class="bg-gradient-to-br from-green-500 to-green-600 text-white border-0">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-xs opacity-75 mt-1">{{ stats.productsStock }} em estoque</p>
            </div>
            <div class="bg-white bg-opacity-20 rounded-full p-3">
              <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <BaseCard class="bg-gradient-to-br from-purple-500 to-purple-600 text-white border-0">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-xs opacity-75 mt-1">{{ stats.activeRaffles }} ativas</p>
            </div>
            <div class="bg-white bg-opacity-20 rounded-full p-3">
              <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <BaseCard class="bg-gradient-to-br from-pink-500 to-pink-600 text-white border-0">
          <div class="flex items-center justify-between">
            <div>
              <h3 class="text-sm font-medium opacity-90 mb-2">Eventos</h3>
              <p class="text-3xl font-bold">{{ stats.eventsCount }}</p>
              <p class="text-xs opacity-75 mt-1">{{ stats.upcomingEvents }} próximos</p>
            </div>
            <div class="bg-white bg-opacity-20 rounded-full p-3">
              <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <BaseCard>
          ...
          <div class="space-y-4">
            <div v-for="goal in topGoals" :key="goal.id" class="border-b border-gray-100 pb-3 last:border-0">
              <div class="flex justify-between items-center mb-2">
                <span class="text-sm font-medium text-gray-900 truncate max-w-xs">{{ goal.title }}</span>
                <span class="text-sm font-bold text-tyler-blue">{{ goal.progress }}%</span>
              </div>
              <ProgressBar :progress="goal.progress" size="sm" />
              <div class="flex justify-between items-center mt-1">
                <span class="text-xs text-gray-500">{{ formatCurrency(goal.currentAmount) }}</span>
                <span class="text-xs text-gray-500">{{ formatCurrency(goal.targetAmount) }}</span>
              </div>
            </div>
          </div>
        </BaseCard>
        ...
        <BaseCard>
          ...
          <div class="space-y-4">
            <div v-for="raffle in activeRafflesList" :key="raffle.id" class="border-b border-gray-100 pb-3 last:border-0">
              <div class="flex items-center gap-3">
                <img 
                ...
                <div class="flex-1">
                  <div class="flex justify-between items-center mb-1">
                    <span class="text-sm font-medium text-gray-900">{{ raffle.prize }}</span>
                    <Badge variant="success">{{ raffle.progress }}%</Badge>
                  </div>
                  <div class="text-xs text-gray-500">
                    {{ raffle.soldTickets }} / {{ raffle.totalTickets }} bilhetes vendidos
                  </div>
                  <ProgressBar :progress="raffle.progress" size="sm" color="pink" class="mt-2" />
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <BaseCard>
          ...
          <div class="space-y-3">
            <div v-for="product in topProducts" :key="product.id" class="flex items-center gap-3 border-b border-gray-100 pb-3 last:border-0">
              ...
              <div class="text-right">
                <div class="text-sm font-bold text-tyler-blue">{{ formatCurrency(product.price) }}</div>
        ...
        <BaseCard>
          ...
          <div class="space-y-3">
            <div v-for="event in upcomingEventsList" :key="event.id" class="flex items-center gap-3 border-b border-gray-100 pb-3 last:border-0">
              <img :src="event.imageUrl" :alt="event.title" class="w-10 h-10 rounded object-cover" />
              <div class="flex-1">
                <div class="text-sm font-medium text-gray-900">{{ event.title }}</div>
                <div class="text-xs text-gray-500">{{ formatDate(event.date) }} · {{ event.location }}</div>
              </div>
              <div class="text-right">
...
const stats = computed(() => ({
  ...
  activeRaffles: rafflesStore.raffles.filter(r => r.status === 'ACTIVE').length,
