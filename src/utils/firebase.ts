...
function isValidFirebaseConfig(config: any): boolean {
  ...
  if (config.projectId === 'tyler-project' || config.apiKey === 'your_api_key_here') {
    return false;
  }
  
  // Verificar se não é chave do Google Console (erro comum)
  if (config.apiKey && config.apiKey.startsWith('GOCSPX-')) {
    console.error('❌ Erro: Você está usando uma chave do Google Console, não do Firebase!');
    console.error('🔧 Solução: Vá para console.firebase.google.com e obtenha a chave correta');
...
if (!isFirebaseConfigValid) {
  console.warn('⚠️ Firebase não configurado corretamente. Usando modo de desenvolvimento.');
...
export { app, auth, signInWithEmailAndPassword, signInWithPopup, GoogleAuthProvider, signOut, onAuthStateChanged };
export type { User };
...
export const firebaseService = {
  ...
  async signInWithGoogle() {
    ...
    if (!auth || !googleProvider) {
      console.error('❌ Firebase não configurado:', { auth: !!auth, provider: !!googleProvider });
      return {
        success: false,
