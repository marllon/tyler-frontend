<template>
  <div class="bg-white shadow rounded-lg p-6">
    ...
    <div class="border-t pt-4">
      <h4 class="text-md font-medium text-gray-800 mb-3">Status da Configuração</h4>
      ...
      <div class="space-y-2">
        ...
        <div class="flex items-center justify-between">
          ...
          <span class="text-sm font-medium" :class="currentUser?.role === 'super-admin' ? 'text-purple-600' : 'text-blue-600'">
        ...
        <div class="flex items-center justify-between">
          ...
          <span class="text-sm font-medium" :class="configStatus.valid ? 'text-green-600' : 'text-red-600'">
      ...
      <div v-if="configStatus.issues.length > 0" class="mt-3 p-3 bg-red-50 border border-red-200 rounded-md">
        <h5 class="text-sm font-medium text-red-800 mb-1">Problemas encontrados:</h5>
        <ul class="text-sm text-red-700 space-y-1">
          <li v-for="issue in configStatus.issues" :key="issue">• {{ issue }}</li>
        </ul>
      </div>
    </div>
      ...
      <div class="flex">
        ...
        <div class="ml-3">
          ...
          <p class="text-sm text-yellow-700 mt-1">
            Tenha cuidado ao remover administradores. Certifique-se de que sempre há pelo menos um super-admin ativo.
          </p>
        </div>
      </div>
    </div>
...
async function updateConfigStatus() {
  try {
    ...
    configStatus.value = { valid: false, issues: ['Erro ao validar configuração'] };
  }
}
...
async function loadAuthorizedEmails() {
  if (currentUser.value) {
    loading.value = true;
    try {
      const emails = await authorizationService.getAuthorizedEmails(currentUser.value.role);
...
function getRoleDisplayName(email: string): string {
  ...
  const role = email.includes('tyler') || email.includes('admin@tylerlimaeler.org') 
    ? 'super-admin' 
    : 'admin';
...
async function addEmail() {
  ...
  try {
    ...
    const authResult = await authorizationService.checkEmailAuthorization(newEmail.value);
...
async function removeEmail(email: string) {
  ...
  if (!confirm(`Tem certeza que deseja remover ${email} da lista de administradores?`)) {
    return;
  }
