  ...
  <div class="flex gap-4 min-h-[300px]">
    ...
    <div class="w-20 flex flex-col space-y-2">
      ...
      >
        ...
        <!-- Badge principal -->
        <div v-if="image.isPrimary || image.isMain || index === 0" 
    ...
    <div class="flex-1 max-w-md">
      ...
      <div v-if="selectedImage" class="relative aspect-[4/3] bg-white border-2 border-gray-200 rounded-lg overflow-hidden">
        <img 
          ...
          <div class="text-white">
            ...
            <div class="flex gap-2 mt-2">
              <button
                ...
                :class="{
                  'bg-blue-800': selectedImage.isPrimary || selectedImage.isMain || selectedIndex === 0
                }"
              >
                {{ (selectedImage.isPrimary || selectedImage.isMain || selectedIndex === 0) ? 'Principal' : 'Tornar Principal' }}
              </button>
              <button
      ...
      <div v-else class="aspect-[4/3] bg-gray-50 border-2 border-dashed border-gray-300 rounded-lg flex flex-col items-center justify-center text-center p-6">
        ...
        <h4 class="text-md font-medium text-gray-900 mb-2">Adicione imagens do produto</h4>
        <p class="text-gray-500 text-sm mb-3">Clique no botão + à esquerda para adicionar imagens</p>
        <label 
          for="image-upload"
...
function handleFileUpload(event: Event) {
  ...
  Array.from(files).slice(0, Math.min(5, props.maxFiles - allImages.value.length)).forEach((file, index) => {
  ...
  // Selecionar a primeira imagem nova adicionada
  if (newImages.length > 0) {
    selectedIndex.value = allImages.value.length - newImages.length
  }
  
  // Limpar input e loading
  target.value = ''
...
function deleteImage(index: number) {
  ...
  // Ajustar selectedIndex se necessário
  if (selectedIndex.value >= allImages.value.length) {
    selectedIndex.value = Math.max(0, allImages.value.length - 1)
  }

  // Se deletou a imagem principal, tornar a primeira como principal
  if ((imageToDelete.isPrimary || imageToDelete.isMain) && allImages.value.length > 0) {
...
<style scoped>
/* Scroll customizado para thumbnails se necessário */
.thumbnail-scroll {
